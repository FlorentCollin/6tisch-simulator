...
class DiscreteEventEngine(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            while self.goOn:
                with self.dataLock:
                    ...
                    for intraSlotOrder in intraSlotOrderKeys:
                        for uniqueTag, cb in list(
                                self.events[self.asn][intraSlotOrder].items()):
                            cbs += [cb]
            ...
            output += [
                u'The log file is {0}'.format(self.settings.getOutputFile())
            ]
            output += [u'']
            output += [u'==============================']
    ...
    def scheduleAtAsn(self, asn, cb, uniqueTag, intraSlotOrder):
        ...
        with self.dataLock:
            if asn not in self.events:
                ...
                self.events[asn][intraSlotOrder] = (OrderedDict([(uniqueTag,
                                                                  cb)]))
    ...
    def scheduleIn(self, delay, cb, uniqueTag, intraSlotOrder):
        ...
        with self.dataLock:
            asn = int(self.asn +
                      (float(delay) / float(self.settings.tsch_slotDuration)))
    ...
    def _actionEndSlotframe(self):
        ...
        slotframe_iteration = int(
            old_div(self.asn, self.settings.tsch_slotframeLength))
        # print
        if self.verbose:
            print(u'   slotframe_iteration: {0}/{1}'.format(
                slotframe_iteration,
...
class SimEngine(DiscreteEventEngine):
    ...
    def _init_additional_local_variables(self):
        ...
        self.motes = [
            Mote.Mote.Mote(id, eui64) for id, eui64 in zip(
                list(range(self.settings.exec_numMotes)), eui64_table)
        ]
        ...
        # log the random seed
        self.log(SimLog.LOG_SIMULATOR_RANDOM_SEED,
    ...
    def _routine_thread_started(self):
        # log
        self.log(SimLog.LOG_SIMULATOR_STATE, {
            u'name': self.name,
            u'state': u'started'
        })
        # schedule end of simulation
        self.scheduleAtAsn(
            asn=self.settings.tsch_slotframeLength *
            self.settings.exec_numSlotframesPerRun,
            cb=self._actionEndSim,
