...
class SchedulingFunctionMSF(SchedulingFunctionBase):
    ...
    def indication_tx_cell_elapsed(self, cell, sent_packet):
        ...
        if (preferred_parent and (cell.mac_addr == preferred_parent)
                and (cell.options == [d.CELLOPTION_TX])):
            ...
            if d.MSF_MAX_NUMCELLS <= self.num_tx_cells_elapsed:
                tx_cell_utilization = (self.num_tx_cells_used /
    ...
    def indication_parent_change(self, old_parent, new_parent):
        ...
        if old_parent is None:
            ...
            dedicated_cells = self.mote.tsch.get_cells(
                ...
                slotframe_handle=self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
            num_tx_cells = len([
                cell for cell in dedicated_cells
        ...
        if new_parent:
            ...
            self._request_adding_cells(neighbor=new_parent,
        ...
        if old_parent:
            cells = self.mote.tsch.get_cells(
                ...
                slotframe_handle=self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
            if len(cells) >= 1:
                self.mote.sixp.send_request(dstMac=old_parent,
    ...
    def get_tx_cells(self, mac_addr):
        ...
        if slotframe:
            ...
            negotiated_tx_cells = [
                cell for cell in cells if cell.options == [d.CELLOPTION_TX]
            ]
    ...
    def get_negotiated_rx_cells(self, mac_addr):
        ...
        if slotframe:
            cells = slotframe.get_cells_by_mac_addr(mac_addr)
            ret = [cell for cell in cells if cell.options == [d.CELLOPTION_RX]]
        else:
            ret = []
        return ret
    ...
    def get_autonomous_tx_cell(self, mac_addr):
        ...
        if slotframe:
            ...
            autonomous_cells = [
                ...
                if ((d.CELLOPTION_TX in cell.options) and (
    ...
    def _handle_rx_cell_elapsed_event(self, used_by_parent):
        ...
        # adapt number of cells if necessary
        rx_cell_utilization = (self.num_rx_cells_used /
    ...
    def _adapt_to_traffic(self, neighbor, cell_opt):
        ...
        if cell_opt == self.TX_CELL_OPT:
            if d.MSF_LIM_NUMCELLSUSED_HIGH < self.tx_cell_utilization:
                ...
                self._request_adding_cells(neighbor=neighbor, num_tx_cells=1)
                ...
                if len(tx_cells) > 1:
                    self.retry_count[neighbor] = 0
                    self._request_deleting_cells(neighbor=neighbor,
                                                 num_cells=1,
    ...
    def _housekeeping_collision(self):
        ...
        tx_cell_list = [
            cell for cell in self.mote.tsch.get_cells(
                preferred_parent, self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
            if cell.options == [d.CELLOPTION_TX]
        ]
        ...
        tx_cell_list = {
            cell.slot_offset: cell
            for cell in tx_cell_list if (d.MSF_MIN_NUM_TX < cell.num_tx)
        }
        ...
        pdr_list = {
            slotOffset: pdr(cell)
            for slotOffset, cell in list(tx_cell_list.items())
        }
        if len(pdr_list) > 0:
            ...
            relocation_cell_list = [
                ...
                } for slotOffset, pdr in list(pdr_list.items())
                if (d.MSF_RELOCATE_PDRTHRES < (highest_pdr - pdr))
            ]
            if (len(relocation_cell_list) > 0
                    and self.retry_count[preferred_parent] == -1):
                # reset retry counter
        ...
        try:
            for cell in cell_list:
                self.mote.tsch.addCell(
                    ...
                    slotframe_handle=self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
            if (cell_options == [d.CELLOPTION_TX]
                    and self.get_autonomous_tx_cell(neighbor)):
    ...
    def _relocate_cells(self, neighbor, src_cell_list, dst_cell_list,
                        cell_options):
    ...
    def _get_available_slots(self):
        return list(
            set(
                self.mote.tsch.get_available_slots(
                    self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)) -
            self.locked_slots)
    ...
    def _create_occupied_cell_list(self, neighbor, cell_options,
                                   cell_list_len):
        occupied_cells = [
            cell for cell in self.mote.tsch.get_cells(
                neighbor, self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
    ...
    def _are_cells_allocated(self, peerMac, cell_list, cell_options):
        # collect allocated cells
        assert cell_options in [self.TX_CELL_OPT, self.RX_CELL_OPT]
        allocated_cells = [
            cell for cell in self.mote.tsch.get_cells(
                peerMac, self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
            if cell.options == cell_options
        ]
    ...
    def _request_adding_cells(self, neighbor, num_tx_cells=0, num_rx_cells=0):
        ...
        if num_tx_cells > 0:
            ...
            return
        # prepare cell_list
        cell_list = self._create_available_cell_list(
            self.DEFAULT_CELL_LIST_LEN)
        if len(cell_list) == 0:
            # we don't have available cells right now
            self.log(SimEngine.SimLog.LOG_MSF_ERROR_SCHEDULE_FULL,
                     {'_mote_id': self.mote.id})
            self.retry_count[neighbor] = -1
            return
        # prepare _callback which is passed to SixP.send_request()
        callback = self._create_add_request_callback(neighbor, num_cells,
                                                     cell_options, cell_list,
                                                     num_tx_cells,
                                                     num_rx_cells)
        # send a request
        self.mote.sixp.send_request(dstMac=neighbor,
    ...
    def _receive_add_request(self, request):
        ...
        slots_in_cell_list = set([c[u'slotOffset'] for c in proposed_cells])
        available_slots = list(
            slots_in_cell_list.intersection(set(self._get_available_slots())))
        # prepare cell_list
        candidate_cells = [
        ...
        if len(candidate_cells) < request[u'app'][u'numCells']:
            ...
            cell_list = random.sample(candidate_cells,
        ...
        if len(available_slots) > 0:
            ...
            def callback(event, packet):
                if event == d.SIXP_CALLBACK_EVENT_MAC_ACK_RECEPTION:
                    ...
                    if request[u'app'][u'cellOptions'] == self.TX_CELL_OPT:
                        ...
                        raise Exception()
                    self._add_cells(neighbor=peerMac,
                                    cell_list=cell_list,
            ...
            callback = None
        # send a response
        self.mote.sixp.send_response(dstMac=peerMac,
                                     return_code=code,
                                     cellList=cell_list,
                                     callback=callback)

    def _create_add_request_callback(self, neighbor, num_cells, cell_options,
                                     cell_list, num_tx_cells, num_rx_cells):
        def callback(event, packet):
            if event == d.SIXP_CALLBACK_EVENT_PACKET_RECEPTION:
                ...
                if packet[u'app'][u'code'] == d.SIXP_RC_SUCCESS:
                    ...
                    self._add_cells(neighbor=neighbor,
                    ...
                    remaining_cells = num_cells - len(
                        packet[u'app'][u'cellList'])
                    ...
                    self.retry_count[neighbor] = 0
                    self._request_adding_cells(neighbor=neighbor,
    ...
    def _create_delete_request_callback(self, neighbor, num_cells,
                                        cell_options):
        def callback(event, packet):
            if ((event == d.SIXP_CALLBACK_EVENT_PACKET_RECEPTION) and
                (packet[u'app'][u'msgType'] == d.SIXP_MSG_TYPE_RESPONSE)):
                self.retry_count[neighbor] = -1
                if packet[u'app'][u'code'] == d.SIXP_RC_SUCCESS:
                    self._delete_cells(neighbor=neighbor,
                                       cell_list=packet[u'app'][u'cellList'],
                                       cell_options=cell_options)
                else:
                    # TODO: request doesn't succeed; how should we do?
                ...
                if self.retry_count[neighbor] == self.MAX_RETRY:
                    ...
                    # retry
                    self.retry_count[neighbor] += 1
                    self._request_deleting_cells(neighbor, num_cells,
                                                 cell_options)
            else:
                # ignore other events
                pass
    ...
    def _request_relocating_cells(self, neighbor, cell_options,
                                  num_relocating_cells, cell_list):
        ...
        def callback(event, packet):
            if event == d.SIXP_CALLBACK_EVENT_PACKET_RECEPTION:
                ...
                if packet[u'app'][u'code'] == d.SIXP_RC_SUCCESS:
                    ...
                    _num_relocating_cells = (num_relocating_cells + num_cells -
                                             num_relocations)
                    _cell_list = (cell_list +
                                  relocation_cell_list[num_relocations:])
                    # start another transaction
                    self.retry_count[neighbor] = 0
                    self._request_relocating_cells(
                        neighbor=neighbor,
                ...
                if self.retry_count[neighbor] == self.MAX_RETRY:
                    ...
                    # retry
                    self.retry_count[neighbor] += 1
                    self._request_relocating_cells(neighbor, cell_options,
    ...
    def _receive_relocate_request(self, request):
        ...
        if ((self._are_cells_allocated(peerMac=peerMac,
                                       ...
                                       cell_options=our_cell_options) is True)
                and (num_cells <= len(candidate_cells))):
            # find available cells in the received candidate cell list
            slots_in_slotframe = set(
                range(0, self.settings.tsch_slotframeLength))
            slots_in_use = set(
                self.mote.tsch.get_busy_slots(
                    self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS))
            candidate_slots = set([c[u'slotOffset'] for c in candidate_cells])
            available_slots = list(
                candidate_slots.intersection(set(self._get_available_slots())))
            code = d.SIXP_RC_SUCCESS
            cell_list = []
            if available_slots:
                # prepare response
