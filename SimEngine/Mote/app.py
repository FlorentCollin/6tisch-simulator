...
class AppBase(object):
    ...
    def recvPacket(self, packet):
        ...
        """
        # log and mote stats
        self.log(SimEngine.SimLog.LOG_APP_RX, {
    ...
    def _send_packet(self, dstIp, packet_length):
        ...
        if self.mote.clear_to_send_EBs_DATA() == False:
            return
        # create
        packet = self._generate_packet(dstIp=dstIp,
                                       packet_type=d.PKT_TYPE_DATA,
                                       packet_length=packet_length)
        # log
        self.log(SimEngine.SimLog.LOG_APP_TX, {
            u'_mote_id': self.mote.id,
            u'packet': packet,
        })
        # send
...
class AppRoot(AppBase):
    ...
    def _send_ack(self, destination, packet_length=None):
        ...
        self._send_packet(dstIp=destination, packet_length=packet_length)
...
class AppPeriodic(AppBase):
    ...
    def _schedule_transmission(self):
        ...
        if self.sending_first_packet:
            ...
            delay = self.settings.tsch_slotDuration + (
                self.settings.app_pkPeriod * random.random())
            self.sending_first_packet = False
        else:
            # compute random delay
            assert self.settings.app_pkPeriodVar < 1
            delay = self.settings.app_pkPeriod * (1 + random.uniform(
                -self.settings.app_pkPeriodVar, self.settings.app_pkPeriodVar))
        # schedule
        self.engine.scheduleIn(
            delay=delay,
            cb=self._send_a_single_packet,
            uniqueTag=(u'AppPeriodic',
    ...
    def _send_a_single_packet(self):
        if self.mote.rpl.dodagId == None:
            ...
            self.sending_first_packet = True
            return
        self._send_packet(dstIp=self.mote.rpl.dodagId,
