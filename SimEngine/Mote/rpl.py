...
class Rpl(object):
    ...
    def action_receiveDAO(self, packet):
        ...
        self.addParentChildfromDAOs(parent_addr=packet[u'app'][u'parent_addr'],
...
class RplOFBase(object):
    ...
    def reset(self):
        ...
        self.rpl.indicate_preferred_parent_change(
            old_preferred=old_parent_mac_addr, new_preferred=None)
...
class RplOF0(RplOFBase):
    ...
    # 5.1.1. of RFC 8180.
    ETX_GOOD_LINK = 2
    PARENT_SWITCH_RANK_INCREASE_THRESHOLD = (((3 * ETX_GOOD_LINK) - 2) *
    ...
    def update(self, dio):
        ...
        # invalidate our rank
        if ((self.preferred_parent == neighbor)
                and (rank == d.RPL_INFINITE_RANK)):
            self.rank = None
    ...
    def update_etx(self, cell, mac_addr, isACKed):
        ...
        elif ((neighbor[u'numTxAck'] == 0)
              and (self.MAX_NUM_OF_CONSECUTIVE_FAILURES_WITHOUT_SUCCESS <=
                   neighbor[u'numTx'])):
            # set invalid ETX
    ...
    def _update_neighbor_rank_increase(self, neighbor):
        ...
        if step_of_rank is None:
            ...
            neighbor[
                u'rank_increase'] = step_of_rank * d.RPL_MINHOPRANKINCREASE
        if neighbor == self.preferred_parent:
            self.rank = self._calculate_rank(self.preferred_parent)

    def _calculate_rank(self, neighbor):
        if ((neighbor is None) or (neighbor[u'advertised_rank'] is None)
                or (neighbor[u'rank_increase'] is None)):
            return None
    ...
    def _update_preferred_parent(self):
        if ((self.preferred_parent is not None)
                and (self.preferred_parent[u'advertised_rank'] is not None)
                and (self.rank is not None)
                and ((self.preferred_parent[u'advertised_rank'] - self.rank) <
                     d.RPL_PARENT_SWITCH_RANK_THRESHOLD)
                and (self.preferred_parent[u'rank_increase'] <
        ...
        if ((new_parent is not None)
                and (new_parent != self.preferred_parent)):
        ...
        elif ((new_parent is None) and (self.preferred_parent is not None)):
...
class RplOFBestLinkPDR(RplOF0):
    ...
    def parents(self):
        ...
        for neighbor in self.neighbors:
            ...
            while parent_mote.dagRoot is False:
                ...
                if ((parent_id is None) or (parent_id == self.mote.id)):
    ...
    def update_etx(self, cell, mac_addr, isACKed):
        ...
        self._update_preferred_parent()
        if ((previous_parent == self.NONE_PREFERRED_PARENT)
                and (previous_parent != self.preferred_parent)):
            # rejoin the DODAG
            self.rpl.join_dodag()
    ...
    def _calculate_rank(neighbor):
        ...
        if ((neighbor[u'rank'] == d.RPL_INFINITE_RANK)
                or (neighbor[u'mean_link_pdr'] == 0)):
            rank = d.RPL_INFINITE_RANK
        else:
    ...
    def _update_preferred_parent(self):
        ...
        if ((new_preferred_parent != self.NONE_PREFERRED_PARENT)
                and (new_rank == d.RPL_INFINITE_RANK)
                and (self.preferred_parent != self.NONE_PREFERRED_PARENT)):
        ...
        if new_preferred_parent != self.preferred_parent:
            if ((new_preferred_parent == self.NONE_PREFERRED_PARENT) or
                (d.RPL_PARENT_SWITCH_RANK_THRESHOLD <
                ...
                if ((self._calculate_rank(old_preferred_parent)
                     == d.RPL_INFINITE_RANK)
                        and (old_preferred_parent in self.neighbors)):
    ...
    def _update_mean_link_pdr(self, neighbor):
        ...
        neighbor[u'mean_link_pdr'] = numpy.mean([
            ...
            for src_id, dst_id in [(
                self.mote.id,
                neighbor[u'mote_id']), (neighbor[u'mote_id'], self.mote.id)]
        ])
    ...
    def _update_mean_link_rssi(self, neighbor):
        ...
        neighbor[u'mean_link_rssi'] = numpy.mean([
            self.connectivity.get_rssi(src_id=self.mote.id,
    ...
    def _find_best_parent(self):
        ...
        self.neighbors = sorted(
            self.neighbors,
            key=lambda e:
            (self._calculate_rank(e), e[u'mean_link_rssi'], e[u'mote_id']))
