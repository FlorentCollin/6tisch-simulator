...
class Sixlowpan(object):
    ...
    def sendPacket(self, packet):
        ...
        # log
        self.log(SimEngine.SimLog.LOG_SIXLOWPAN_PKT_TX, {
        ...
        if goOn:
            if ((self.mote.dagRoot)
                    and ((netaddr.IPAddress(packet[u'net'][u'srcIp']).words[0]
                          & 0xFE80) != 0xFE80)):
                sourceRoute = self.mote.rpl.computeSourceRoute(
                    packet[u'net'][u'dstIp'])
                if sourceRoute == None:
    ...
    def recvPacket(self, packet):
        ...
        goOn = True
        # log
        self.log(SimEngine.SimLog.LOG_SIXLOWPAN_PKT_RX, {
            u'_mote_id': self.mote.id,
            u'packet': packet,
        })
        ...
        if goOn:
            ...
            # source routing header
                ...
                packet[u'net'][u'dstIp'] = packet[u'net'][u'sourceRoute'].pop(
                    0)
        ...
        if goOn:
            if (packet[u'type'] !=
                    d.PKT_TYPE_FRAG  # in case of fragment forwarding
                    and
                ((self.mote.is_my_ipv6_addr(packet[u'net'][u'dstIp'])) or
                 (packet[u'mac'][u'dstMac'] == d.BROADCAST_ADDRESS))):
                # packet for me
                # dispatch to upper component
                if packet[u'type'] in [
                        d.PKT_TYPE_JOIN_REQUEST, d.PKT_TYPE_JOIN_RESPONSE
                ]:
                ...
                if (((u'downward' not in packet[u'net']) or
                     (packet[u'net'][u'downward'] is False))
                        and (preferred_parent_mac_addr is not None) and
                    (packet[u'mac'][u'srcMac'] == preferred_parent_mac_addr)):
                    # we received an upward packet from our parent
                    if ((u'rank_error' in packet[u'net'])
                            and (packet[u'net'][u'rank_error'] is True)):
    ...
    def forward(self, rxPacket):
        ...
        goOn = True
        if ((u'hop_limit' in rxPacket[u'net'])
                and (rxPacket[u'net'][u'hop_limit'] < 2)):
        ...
        # log
        if goOn:
            self.log(SimEngine.SimLog.LOG_SIXLOWPAN_PKT_FWD, {
                u'_mote_id': self.mote.id,
                u'packet': fwdPacket,
            })
        ...
        if (dst_ip_addr.words[0] & 0xFF00) == 0xFF00:
            ...
            if self.mote.rpl.dodagId is None:
                ...
                mac_addr = str(self.mote.tsch.join_proxy)
                  ...
                  or ((u'downward' in packet[u'net']) and
...
class Fragmentation(object):
    ...
    def fragmentPacket(self, packet):
        ...
        returnVal = []
        if self.settings.tsch_max_payload_len < packet[u'net'][
                u'packet_length']:
            ...
            number_of_fragments = int(
                math.ceil(
                    float(packet[u'net'][u'packet_length']) /
                    self.settings.tsch_max_payload_len))
            datagram_offset = 0
            for i in range(0, number_of_fragments):
                ...
                if i == 0:
                    ...
                    if u'sourceRoute' in packet[u'net']:
                        fragment[u'net'][u'sourceRoute'] = copy.deepcopy(
                            packet[u'net'][u'sourceRoute'])
                ...
                if ((i == 0) and ((packet[u'net'][u'packet_length'] %
                                   self.settings.tsch_max_payload_len) > 0)):
                    # slop is in the first fragment
                    fragment[u'net'][u'packet_length'] = packet[u'net'][
                        u'packet_length'] % self.settings.tsch_max_payload_len
                else:
                    fragment[u'net'][
                        u'packet_length'] = self.settings.tsch_max_payload_len
                ...
                returnVal += [fragment]
                # log
                self.log(SimEngine.SimLog.LOG_SIXLOWPAN_FRAG_GEN, {
                    u'_mote_id': self.mote.id,
                    u'packet': fragment
                })
        else:
    ...
    def reassemblePacket(self, fragment):
        ...
        buffer_lifetime = old_div(d.SIXLOWPAN_REASSEMBLY_BUFFER_LIFETIME,
        ...
        if (srcMac not in self.reassembly_buffers) or (
                incoming_datagram_tag not in self.reassembly_buffers[srcMac]):
            ...
            if not self.mote.dagRoot:
                ...
                for i in self.reassembly_buffers:
                    total_reassembly_buffers_num += len(
                        self.reassembly_buffers[i])
                if total_reassembly_buffers_num == self.settings.sixlowpan_reassembly_buffers_num:
                    # no room for a new entry
                    self.mote.drop_packet(
                        packet=fragment,
                        reason=SimEngine.SimLog.
                        DROPREASON_REASSEMBLY_BUFFER_FULL,
                    )
                    return
        ...
        if datagram_offset not in [
                x[u'datagram_offset'] for x in self.reassembly_buffers[srcMac]
            [incoming_datagram_tag][u'fragments']
        ]:
            if fragment[u'net'][u'datagram_offset'] == 0:
                ...
                del self.reassembly_buffers[srcMac][incoming_datagram_tag][
                    u'net'][u'datagram_size']
                del self.reassembly_buffers[srcMac][incoming_datagram_tag][
                    u'net'][u'datagram_offset']
                del self.reassembly_buffers[srcMac][incoming_datagram_tag][
                    u'net'][u'datagram_tag']
            self.reassembly_buffers[srcMac][incoming_datagram_tag][
                u'fragments'].append({
                    u'datagram_offset':
                    datagram_offset,
                    u'fragment_length':
                    fragment[u'net'][u'packet_length']
                })
        else:
        ...
        total_fragment_length = sum([
            f[u'fragment_length'] for f in self.reassembly_buffers[srcMac]
            [incoming_datagram_tag][u'fragments']
        ])
        ...
        packet[u'net'] = copy.deepcopy(
            self.reassembly_buffers[srcMac][incoming_datagram_tag][u'net'])
    ...
    def _delete_expired_reassembly_buffer(self):
        ...
        for srcMac in list(self.reassembly_buffers.keys()):
            for incoming_datagram_tag in list(
                    self.reassembly_buffers[srcMac].keys()):
                # delete expired reassembly buffer
                if self.reassembly_buffers[srcMac][incoming_datagram_tag][
                        u'expiration'] < self.engine.getAsn():
                    del self.reassembly_buffers[srcMac][incoming_datagram_tag]
...
class FragmentForwarding(Fragmentation):
    ...
    def fragRecv(self, fragment):
        ...
        entry_lifetime = old_div(d.SIXLOWPAN_VRB_TABLE_ENTRY_LIFETIME,
                                 self.settings.tsch_slotDuration)
        ...
        if datagram_offset == 0:
            ...
            if self.mote.dagRoot:
                ...
                total_vrb_table_entry_num = sum(
                    [len(e) for _, e in list(self.vrb_table.items())])
            ...
            if self.mote.is_my_ipv6_addr(fragment[u'net'][u'dstIp']):
                ...
                self.vrb_table[srcMac][incoming_datagram_tag][
                    u'outgoing_datagram_tag'] = None
            else:
                self.vrb_table[srcMac][incoming_datagram_tag][
                    u'dstMac'] = dstMac
                self.vrb_table[srcMac][incoming_datagram_tag][
                    u'outgoing_datagram_tag'] = self._get_next_datagram_tag()
            self.vrb_table[srcMac][incoming_datagram_tag][
                u'expiration'] = self.engine.getAsn() + entry_lifetime
        ...
        if ((u'missing_fragment'
             in self.settings.fragmentation_ff_discard_vrb_entry_policy)
                and (srcMac in self.vrb_table)
                and (incoming_datagram_tag in self.vrb_table[srcMac])):
            if datagram_offset == self.vrb_table[srcMac][
                    incoming_datagram_tag][u'next_offset']:
                self.vrb_table[srcMac][incoming_datagram_tag][
                    u'next_offset'] += packet_length
            else:
        ...
        if (srcMac in self.vrb_table) and (incoming_datagram_tag
                                           in self.vrb_table[srcMac]):
            # VRB entry found!
            if self.vrb_table[srcMac][incoming_datagram_tag][
                    u'outgoing_datagram_tag'] is None:
                ...
                fwdFragment[u'net'][u'datagram_tag'] = self.vrb_table[srcMac][
                    incoming_datagram_tag][u'outgoing_datagram_tag']
        ...
        if ((u'last_fragment'
             in self.settings.fragmentation_ff_discard_vrb_entry_policy)
                and (srcMac in self.vrb_table)
                and (incoming_datagram_tag in self.vrb_table[srcMac])
                and ((datagram_offset + packet_length) == datagram_size)):
            del self.vrb_table[srcMac][incoming_datagram_tag]
            if len(self.vrb_table[srcMac]) == 0:
                del self.vrb_table[srcMac]
        return ret
