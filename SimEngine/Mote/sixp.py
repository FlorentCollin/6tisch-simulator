...
class SixP(object):
    ...
    def clear_transaction_table(self):
        for transaction in [
                self.transaction_table[key] for key in self.transaction_table
        ]:
            transaction.invalidate()

    def recv_packet(self, packet):
        # log
        self.log(SimEngine.SimLog.LOG_SIXP_RX, {
    ...
    def recv_mac_ack(self, packet):
        ...
        if transaction is None:
            ...
            return
        if ((packet[u'app'][u'msgType'] == d.SIXP_MSG_TYPE_REQUEST)
                and (packet[u'app'][u'code'] != d.SIXP_CMD_CLEAR)):
            self.mote.sixp.increment_seqnum(packet[u'mac'][u'dstMac'])
        if (((packet[u'app'][u'msgType'] == d.SIXP_MSG_TYPE_RESPONSE) and
             (transaction.type == d.SIXP_TRANSACTION_TYPE_2_STEP)) or
            ((packet[u'app'][u'msgType'] == d.SIXP_MSG_TYPE_CONFIRMATION) and
             (transaction.type == d.SIXP_TRANSACTION_TYPE_3_STEP))):
            # complete the transaction
            transaction.complete()
            # invoke callback
            transaction.invoke_callback(
                event=d.SIXP_CALLBACK_EVENT_MAC_ACK_RECEPTION, packet=packet)
        else:
            # do nothing
            pass
    ...
    def send_request(self,
        ...
        # create a packet
        packet = self._create_packet(dstMac=dstMac,
        ...
        try:
            ...
            callback(event=d.SIXP_CALLBACK_EVENT_FAILURE, packet=packet)
        else:
            ...
            if packet:
                ...
                callback(event=d.SIXP_CALLBACK_EVENT_FAILURE,
    ...
    def _tsch_enqueue(self, packet):
        self.log(SimEngine.SimLog.LOG_SIXP_TX, {
            u'_mote_id': self.mote.id,
            u'packet': packet
        })
    ...
    def _recv_request(self, request):
        ...
        if transaction is None:
            ...
            try:
                ...
                transaction.start(callback=None, timeout_seconds=None)
            except TransactionAdditionError:
                ...
                self.send_response(dstMac=request[u'mac'][u'srcMac'],
                ...
                if self._is_schedule_inconsistency_detected(transaction):
                    ...
                    # RC_ERR_SEQNUM
                    self.send_response(dstMac=peerMac,
    ...
    def _recv_response(self, response):
        ...
        if transaction is None:
            ...
            # invoke callback
            transaction.invoke_callback(
                event=d.SIXP_CALLBACK_EVENT_PACKET_RECEPTION, packet=response)
    ...
    def _create_packet(self,
        ...
        if msgType == d.SIXP_MSG_TYPE_REQUEST:
            ...
            # command specific
            if ((code == d.SIXP_CMD_ADD) or (code == d.SIXP_CMD_DELETE)):
                ...
                d.SIXP_MSG_TYPE_RESPONSE, d.SIXP_MSG_TYPE_CONFIRMATION
        ]:
            transaction = self._find_transaction(packet)
            assert transaction is not None
            ...
            # value.
            if seqNum is None:
                packet[u'app'][u'seqNum'] = transaction.request[u'app'][
                    u'seqNum']
            else:
            ...
            if ((command == d.SIXP_CMD_ADD) or (command == d.SIXP_CMD_DELETE)
                    or (command == d.SIXP_CMD_RELOCATE)
                    or (command == d.SIXP_CMD_LIST)):
                packet[u'app'][u'cellList'] = cellList
    ...
    def _find_transaction(self, packet):
        ...
        if transaction_key in self.transaction_table:
            ...
            if ((packet[u'app'][u'seqNum'] is None) or
    ...
    def _is_schedule_inconsistency_detected(self, transaction):
        ...
        if ((request[u'app'][u'code'] != d.SIXP_CMD_CLEAR)
                and (((request[u'app'][u'seqNum'] == 0) and
                      (self._get_seqnum(peerMac) != 0)) or
                     ((request[u'app'][u'seqNum'] != 0) and
...
class SixPTransaction(object):
    def __init__(self, mote, request):
        ...
        self.event_unique_tag = u'{0}-{1}-{2}-{3}'.format(
            self.mote.id, self.initiator, self.responder,
    ...
    def get_transaction_key(packet):
        if ((packet[u'app'][u'msgType'] == d.SIXP_MSG_TYPE_REQUEST) or
    ...
    def timeout_handler(self):
        ...
        if self.is_valid is True:
            ...
            self.mote.tsch.remove_packets_in_tx_queue(type=d.PKT_TYPE_SIXP,
                                                      dstMac=self.peerMac)
            ...
            self.invoke_callback(event=d.SIXP_CALLBACK_EVENT_TIMEOUT,
    ...
    def _determine_transaction_type(self):
        if (((self.request[u'app'][u'code'] == d.SIXP_CMD_ADD) and
             (len(self.request[u'app'][u'cellList']) == 0))
                or ((self.request[u'app'][u'code'] == d.SIXP_CMD_DELETE) and
                    (len(self.request[u'app'][u'cellList']) == 0))
                or ((self.request[u'app'][u'code'] == d.SIXP_CMD_RELOCATE) and
    ...
    def _get_default_timeout_seconds(self):
        ...
        one_way_delay = (self.settings.tsch_slotframeLength *
                         self.settings.tsch_slotDuration *
                         self.mote.tsch.max_tx_retries * sum(be_list))
        if ((self.type == d.SIXP_TRANSACTION_TYPE_2_STEP)
                and (self.isInitiator is False)):
            # only round trip need to complete the transaction
            num_round_trips = 1
        elif (((self.type == d.SIXP_TRANSACTION_TYPE_2_STEP) and
               (self.isInitiator is True))
              or ((self.type == d.SIXP_TRANSACTION_TYPE_3_STEP) and
                  (self.isInitiator is False))):
            # two round trips need to complete the transaction
            num_round_trips = 2
        elif ((self.type == d.SIXP_TRANSACTION_TYPE_3_STEP)
              and (self.isInitiator is True)):
