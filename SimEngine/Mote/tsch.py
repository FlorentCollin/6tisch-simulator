...
class Tsch(object):
    def __init__(self, mote):
        ...
        if self.settings.tsch_tx_queue_size >= 0:
            ...
            raise ValueError(u'unsupported tx_queue_size: {0}'.format(
        ...
        if self.settings.tsch_max_tx_retries >= 0:
            ...
            raise ValueError(u'unsupported tsch_max_tx_retries: {0}'.format(
        ...
        self.add_slotframe(slotframe_handle=0,
    ...
    def setIsSync(self, val):
        ...
        if self.isSync:
            # log
            self.log(SimEngine.SimLog.LOG_TSCH_SYNCED, {
            ...
            # log
            self.log(SimEngine.SimLog.LOG_TSCH_DESYNCED, {
                "_mote_id": self.mote.id,
            })
            ...
            self.engine.removeFutureEvent(uniqueTag=(self.mote.id, u'tsch',
                                                     u'wait_secjoin'))
    ...
    def delete_minimal_cell(self):
        ...
        self.deleteCell(slotOffset=0,
                        channelOffset=0,
                        neighbor=None,
                        cellOptions=[
                            d.CELLOPTION_TX, d.CELLOPTION_RX,
                            d.CELLOPTION_SHARED
                        ],
                        slotframe_handle=0)
    ...
    def deleteCell(self,
        ...
        # raises an exception
        cell = self.get_cell(slotOffset, channelOffset, neighbor,
                             slotframe_handle)
    ...
    def droppable_normal_packet_index(self):
        for rindex, packet in enumerate(reversed(self.txQueue)):
            if ((packet[u'mac'][u'priority'] is False)
                    and (self.pktToSend != packet)):
    ...
    def enqueue(self, packet, priority=False):
        ...
        if (goOn and (len(self.txQueue) == self.txQueueSize)
                and ((priority is False)
                     or self.droppable_normal_packet_index is None)):
            # my TX queue is full
            # drop
            self.mote.drop_packet(
                packet=packet, reason=SimEngine.SimLog.DROPREASON_TXQUEUE_FULL)
            # couldn't enqueue
            goOn = False
        # check that I have cell to transmit on
        if goOn:
            shared_tx_cells = [
                cell for cell in self.mote.tsch.get_cells(None)
                if d.CELLOPTION_TX in cell.options
            ]
            dedicated_tx_cells = [
                cell
                for cell in self.mote.tsch.get_cells(packet[u'mac'][u'dstMac'])
                if d.CELLOPTION_TX in cell.options
            ]
            if ((len(shared_tx_cells) == 0)
                    and (len(dedicated_tx_cells) == 0)):
        ...
        if goOn:
            ...
            if priority:
                ...
                if len(self.txQueue) == self.txQueueSize:
                    ...
                    packet_to_drop = self.dequeue_by_index(
                        packet_index_to_drop)
                    self.mote.drop_packet(
                ...
                # add to txQueue
                self.txQueue += [packet]
        if (goOn and packet[u'mac'][u'dstMac'] != d.BROADCAST_ADDRESS
                and isinstance(self.mote.sf, SchedulingFunctionMSF)
                and not self.mote.sf.get_tx_cells(packet[u'mac'][u'dstMac'])):
            # on-demand allocation of autonomous TX cell
            self.mote.sf.allocate_autonomous_tx_cell(packet[u'mac'][u'dstMac'])
        return goOn
    ...
    def dequeue(self, packet):
        if packet in self.txQueue:
            ...
            # do nothing
            pass
        if (packet[u'mac'][u'dstMac'] != d.BROADCAST_ADDRESS
                and isinstance(self.mote.sf, SchedulingFunctionMSF) and not [
                    _pkt for _pkt in self.txQueue
                    if _pkt[u'mac'][u'dstMac'] == packet[u'mac'][u'dstMac']
                ] and self.mote.sf.get_autonomous_tx_cell(
    ...
    def get_first_packet_to_send(self, cell):
        ...
        if dst_mac_addr is None:
            if (len(self.txQueue) == 0 and cell.link_type
                    in [d.LINKTYPE_ADVERTISING, d.LINKTYPE_ADVERTISING_ONLY]):
                # txQueue is empty; we may return an EB
                if (self.mote.clear_to_send_EBs_DATA()
                        and self._decided_to_send_eb()):
    ...
    def get_num_packet_in_tx_queue(self, dst_mac_addr=None):
        if dst_mac_addr is None:
            ...
            return len([
                pkt for pkt in self.txQueue
                if (pkt[u'mac'][u'dstMac'] == dst_mac_addr)
            ])

    def remove_packets_in_tx_queue(self, type, dstMac=None):
        i = 0
        while i < len(self.txQueue):
            if ((self.txQueue[i][u'type'] == type)
                    and ((dstMac is None) or
    ...
    def txDone(self, isACKed, channel):
        ...
        self.log(
            SimEngine.SimLog.LOG_TSCH_TXDONE, {
                ...
                channel,
                u'slot_offset':
                (active_cell.slot_offset if active_cell else None),
                u'channel_offset':
                (active_cell.channel_offset if active_cell else None),
                u'packet':
                self.pktToSend,
                u'isACKed':
                isACKed,
        ...
        if self.pktToSend[u'mac'][u'dstMac'] == d.BROADCAST_ADDRESS:
            ...
            assert self.pktToSend[u'type'] in [
                d.PKT_TYPE_EB, d.PKT_TYPE_DIO, d.PKT_TYPE_DIS
            ]
            ...
            if ((isACKed is True)
                    and (self.pktToSend[u'type'] == d.PKT_TYPE_SIXP)):
                self.mote.sixp.recv_mac_ack(self.pktToSend)
            if active_cell:
                self.mote.rpl.indicate_tx(active_cell,
            ...
            if isACKed:
                ...
                if ((self.pktToSend[u'mac'][u'pending_bit'] is True)
                        and self._is_next_slot_unused()):
                    self._schedule_next_tx_for_pending_bit(
                        self.pktToSend[u'mac'][u'dstMac'], channel)
                else:
                    self.args_for_next_pending_bit_task = None
            else:
                # ... which was NOT ACKed
        ...
        if active_cell:
            ...
            self.mote.sf.indication_tx_cell_elapsed(cell=active_cell,
    ...
    def rxDone(self, packet, channel):
        ...
        if packet:
            ...
            if ((self.getIsSync() is False)
                    and (packet[u'type'] != d.PKT_TYPE_EB)):
                return False  # isACKed
            # abort if I received a frame for someone else
            if ((packet[u'mac'][u'dstMac'] != d.BROADCAST_ADDRESS)
                    and (self.mote.is_my_mac_addr(packet[u'mac'][u'dstMac']) is
                         False)):
                return False  # isACKed
            ...
            if self.clock.source == packet[u'mac'][u'srcMac']:
                ...
                self._reset_synchronization_timer()
            # update schedule stats
            if (self.getIsSync() and active_cell):
            ...
            if self.mote.is_my_mac_addr(packet[u'mac'][u'dstMac']):
                ...
                if packet[u'type'] == d.PKT_TYPE_SIXP:
                    ...
                    raise SystemError()
                if (is_pending_bit_on and self._is_next_slot_unused()):
                ...
                if packet[u'type'] == d.PKT_TYPE_EB:
                    ...
                    assert packet[u'type'] in [d.PKT_TYPE_DIO, d.PKT_TYPE_DIS]
                    self.mote.sixlowpan.recvPacket(packet)
                else:
                    raise SystemError()
            else:
        ...
        if active_cell:
            ...
            self.mote.sf.indication_rx_cell_elapsed(cell=active_cell,
        ...
        for cell in candidate_cells:
            if cell.is_tx_on():
                if ((packet_to_send is None)
                        or (self.get_num_packet_in_tx_queue(
                            packet_to_send[u'mac'][u'dstMac']) <
                    ...
                    if _packet_to_send is not None:
                        if _packet_to_send[u'type'] == d.PKT_TYPE_EB:
                            if (((cell.mac_addr is None) or
                                 (cell.mac_addr == d.BROADCAST_ADDRESS))
                                    and (cell.link_type in [
                                ...
                                cell.is_shared_on()
                                and self._is_retransmission(_packet_to_send)
                                and
                            (u'backoff_remaining_delay' in _packet_to_send) and
                            ...
                            packet_to_send = _packet_to_send
                            active_cell = cell
            if (cell.is_rx_on() and (packet_to_send is None)):
                active_cell = cell
        if ((packet_to_send is not None)
                and (u'backoff_remaining_delay' in packet_to_send)):
    ...
    def _schedule_next_active_slot(self):
        ...
        if not self.isSync:
            self.engine.removeFutureEvent(uniqueTag=(self.mote.id,
                                                     u'_action_active_cell'))
            return
        try:
            tsDiffMin = min([
                slotframe.get_num_slots_to_next_active_cell(asn)
                for _, slotframe in list(self.slotframes.items())
                if (len(slotframe.get_busy_slots()) > 0)
            ])
        except ValueError:
    ...
    def _action_active_cell(self):
        ...
        if len(candidate_cells) == 0:
            ...
            self.active_cell, self.pktToSend = self._select_active_cell(
                candidate_cells)
        if self.active_cell:
            if self.pktToSend is None:
                ...
                self._action_TX(pktToSend=self.pktToSend,
                                channel=self._get_physical_channel(
                                    self.active_cell))
                # update cell stats
        ...
        for cell in candidate_cells:
            ...
            if cell != self.active_cell:
                if cell.is_tx_on():
                    self.mote.sf.indication_tx_cell_elapsed(cell=cell,
                                                            sent_packet=None)
                if cell.is_rx_on():
                    self.mote.sf.indication_rx_cell_elapsed(
                        cell=cell, received_packet=None)
        # schedule the next active slot
        self._schedule_next_active_slot()

    def _action_TX(self, pktToSend, channel):
        # set the pending bit field
        if ((pktToSend[u'mac'][u'dstMac'] != d.BROADCAST_ADDRESS) and (
                # we have more than one packet destined to the same
                # neighbor
                len([
                    packet for packet in self.txQueue if
                    (packet[u'mac'][u'dstMac'] == pktToSend[u'mac'][u'dstMac'])
                ]) > 1) and self._is_next_slot_unused()
                and self.pending_bit_enabled):
    ...
    def _get_physical_channel(self, cell):
        ...
        return self.hopping_sequence[(self.engine.getAsn() +
                                      cell.channel_offset) %
    ...
    def _decided_to_send_eb(self):
        ...
        return ((random.random() < (old_div(prob, n))) and self.iAmSendingEBs)
    ...
    def _create_EB(self):
        ...
        if join_metric is None:
            ...
            # log
            self.log(SimEngine.SimLog.LOG_TSCH_EB_TX, {
                u'_mote_id': self.mote.id,
                u'packet': newEB,
            })
        return newEB
    ...
    def _action_receiveEB(self, packet):
        assert packet[u'type'] == d.PKT_TYPE_EB
        # log
        self.log(SimEngine.SimLog.LOG_TSCH_EB_RX, {
            u'_mote_id': self.mote.id,
            u'packet': packet,
        })
        ...
        if not self.getIsSync():
            ...
            if len(self.received_eb_list) == d.TSCH_NUM_NEIGHBORS_TO_WAIT:
                ...
                assert len(
                    self.received_eb_list) < d.TSCH_NUM_NEIGHBORS_TO_WAIT
    ...
    def _is_retransmission(self, packet):
        ...
        if u'retriesLeft' not in packet[u'mac']:
            ...
            return (packet[u'mac'][u'retriesLeft'] < self.max_tx_retries)
    ...
    def _increase_backoff_exponent(self):
        ...
        # here.
        self.backoff_exponent = min(self.backoff_exponent + 1,
    ...
    def _update_backoff_state(self, isRetransmission, isSharedLink,
                              isTXSuccess, packet):
        if isSharedLink:
            if isTXSuccess:
                ...
                if isRetransmission:
                    ...
                    self._reset_backoff_state()
                packet[
                    u'backoff_remaining_delay'] = self._decide_backoff_delay()
        else:
    ...
    def _send_keep_alive_message(self):
        if self.clock.source is None:
            return
        if ((len(self.txQueue) > 0)
                and (self.txQueue[0][u'mac'][u'dstMac'] == self.clock.source)):
    ...
    def _start_keep_alive_timer(self):
        ...
        if ((self.settings.tsch_keep_alive_interval == 0)
                or (self.mote.dagRoot is True)):
            # do nothing
            pass
        else:
    ...
    def _reset_synchronization_timer(self):
        if ((self.settings.tsch_keep_alive_interval == 0)
            ...
            target_asn = self.engine.getAsn(
            ) + d.TSCH_DESYNCHRONIZED_TIMEOUT_SLOTS
    ...
    def _schedule_next_rx_by_pending_bit(self, channel):
        self.args_for_next_pending_bit_task = {u'channel': channel}
        self.engine.scheduleAtAsn(
            asn=self.engine.getAsn() + 1,
...
class Clock(object):
    ...
    def _initialize_error_rate(self):
        ...
        max_drift = (float(self.settings.tsch_clock_max_drift_ppm) /
                     pow(10, 6))
