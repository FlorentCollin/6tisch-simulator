...
class SecJoin(object):
    ...
    def setIsJoined(self, value):
        if value:
            self.log(SimEngine.SimLog.LOG_SECJOIN_JOINED, {
                u'_mote_id': self.mote.id,
            })
            self.mote.rpl.start()
        elif self._isJoined:
            self.log(SimEngine.SimLog.LOG_SECJOIN_UNJOINED, {
                u'_mote_id': self.mote.id,
            })
            self.mote.rpl.stop()
        else:
            self.log(SimEngine.SimLog.LOG_SECJOIN_FAILED, {
                u'_mote_id': self.mote.id,
            })
    ...
    def startJoinProcess(self):
        ...
        if self.settings.secjoin_enabled:
            ...
            self._request_timeout = self.TIMEOUT_BASE * random.uniform(
                1, self.TIMEOUT_RANDOM_FACTOR)
        ...
        if packet[u'type'] == d.PKT_TYPE_JOIN_REQUEST:
            if self.mote.dagRoot is False:
                ...
                if u'stateless_proxy' in packet[u'app']:
                    app[u'stateless_proxy'] = copy.deepcopy(
                        packet[u'app'][u'stateless_proxy'])
    ...
    def _send_join_request(self):
        # log
        self.log(SimEngine.SimLog.LOG_SECJOIN_TX, {
            u'_mote_id': self.mote.id,
        })
        # create join request
        joinRequest = {
        ...
        target_asn = (self.engine.getAsn() + int(
            old_div(self._request_timeout, self.settings.tsch_slotDuration)))
        self.engine.scheduleAtAsn(asn=target_asn,
