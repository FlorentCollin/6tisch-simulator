...
class Connectivity(object):
    ...
    def _get_listener_id_list(self, channel):
        returnVal = []
        for mote in self.engine.motes:
            if ((mote.radio.state == d.RADIO_STATE_RX)
                    and (mote.radio.channel == channel)):
                returnVal.append(mote.id)
        return returnVal

    def _compute_pdr_with_interference(self, listener_id, lockon_transmission,
                                       interfering_transmissions):
        # shorthand
        ...
        lockon_pdr = self.get_pdr(src_id=lockon_tx_mote_id,
                                  dst_id=listener_id,
                                  channel=channel)
        ...
        for src_id in self._matrix:
            ...
            for dst_id in self._matrix[src_id]:
                if src_id == dst_id:
                    ...
                    line += [
                        str(self._matrix[src_id][dst_id][channel][u'pdr'])
                    ]
            line = u'\t|'.join(line)
            output += [line]
        output = u'\n'.join(output)
    ...
    def _additional_initialization(self):
        ...
        for child_id in self.mote_id_list:
            if parent_id is not None:
                for channel in d.TSCH_HOPPING_SEQUENCE[:self.num_channels]:
                    self.set_pdr_both_directions(child_id, parent_id, channel,
                                                 perfect_pdr)
                    self.set_rssi_both_directions(child_id, parent_id, channel,
                                                  perfect_rssi)
            parent_id = child_id
...
class ConnectivityMatrixK7(ConnectivityMatrixBase):
    ...
    def _additional_initialization(self):
        ...
        with gzip.open(self.settings.conn_trace, u'r') as tracefile:
            ...
            self.csv_header = tracefile.readline().decode(
                'utf-8').strip().split(u',')
            self.start_date = dt.datetime.strptime(
                self.trace_header[u'start_date'], u'%Y-%m-%dT%H:%M:%S.%f')
            stop_date = dt.datetime.strptime(self.trace_header[u'stop_date'],
                                             u'%Y-%m-%dT%H:%M:%S.%f')
            # check if the simulation settings match the trace file
            if self.settings.exec_numMotes != self.trace_header[u'node_count']:
                print(
                    u'Wrong configuration. exec_numMotes is {0}, should be {1}'
                    .format(self.settings.exec_numMotes,
                            self.trace_header[u'node_count']))
                assert (self.settings.exec_numMotes ==
                        self.trace_header[u'node_count'])
    ...
    def _set_connectivity(self, row):
        ...
        """
        for channel in d.TSCH_HOPPING_SEQUENCE[:self.num_channels]:
            if ((row[u'channel'] is None) or (row[u'channel'] == channel)):
                self.set_pdr(row[u'src_id'], row[u'dst_id'], channel,
                             row[u'pdr'])
                self.set_rssi(row[u'src_id'], row[u'dst_id'], channel,
                              row[u'mean_rssi'])
    ...
    def _parse_line(self, line):
        ...
        row[u'datetime'] = dt.datetime.strptime(row[u'datetime'],
                                                u'%Y-%m-%dT%H:%M:%S.%f')
        # rssi
        ...
        row[u'asn'] = int(time_delta.total_seconds() /
...
class ConnectivityMatrixRandom(ConnectivityMatrixBase):
        ...
        for target_mote_id in self.mote_id_list:
            ...
            while mote_is_deployed is False:
                ...
                if target_mote_id == 0:
                    ...
                    continue
                coordinate = (square_side * random.random(),
                ...
                for deployed_mote_id in self.coordinates:
                    ...
                    self.set_pdr_both_directions(target_mote_id,
                                                 deployed_mote_id,
                                                 base_channel, pdr)
                    self.set_rssi_both_directions(target_mote_id,
                                                  deployed_mote_id,
                                                  base_channel, rssi)
                    if init_min_pdr <= pdr:
                        good_pdr_count += 1
                # determine whether we deploy this mote or not
                if (((len(self.coordinates) <= init_min_neighbors) and
                     (len(self.coordinates) == good_pdr_count))
                        or ((init_min_neighbors < len(self.coordinates)) and
                    ...
                    for deployed_mote_id in list(self.coordinates.keys()):
                        rssi = self.get_rssi(target_mote_id, deployed_mote_id,
                                             base_channel)
                        pdr = self.get_pdr(target_mote_id, deployed_mote_id,
                                           base_channel)
                        for channel in d.TSCH_HOPPING_SEQUENCE[:self.
                                                               num_channels]:
                            if channel == base_channel:
                                ...
                                self.set_pdr_both_directions(
                                    target_mote_id, deployed_mote_id, channel,
                                    pdr)
                    ...
                    for deployed_mote_id in self.coordinates:
                        self._clear_rssi(target_mote_id, deployed_mote_id,
                                         base_channel)
                        self._clear_pdr(target_mote_id, deployed_mote_id,
                                        base_channel)
    ...
    def _clear_rssi(self, mote_id_1, mote_id_2, channel):
        self.set_rssi_both_directions(mote_id_1, mote_id_2, channel,
...
class PisterHackModel(object):
    ...
    def compute_mean_rssi(self, src, dst):
        # distance in meters
        distance = self._get_distance_in_meters(src[u'coordinate'],
                                                dst[u'coordinate'])
        ...
        pr = (src[u'mote'].radio.txPower + src[u'mote'].radio.antennaGain +
    ...
    def compute_rssi(self, src, dst):
        ...
        rssi = (mu + random.uniform(old_div(-self.PISTER_HACK_LOWER_SHIFT, 2),
