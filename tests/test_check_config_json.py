...
@pytest.fixture(params=[
    'success', 'not found', 'not json', 'missing setting',
    'unsupported setting'
])
...
def test_check_config_json(fixture_test_type):
    ...
    config_json = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                               '..', 'bin/config.json')
    ...
    check_config_json = os.path.join(root_path, 'bin/check_config_json.py')
    resulting_exception = None
    popen = None
    try:
        if fixture_test_type == 'success':
            subprocess.check_output([check_config_json, '-c', config_json])
        elif fixture_test_type == 'not found':
            config_json = os.path.join(os.path.dirname(__file__),
                                       'non_existent_dir', 'config.json')
            expected_error_message = 'config.json is not found'
            popen = subprocess.Popen([check_config_json, '-c', config_json],
            ...
            popen = subprocess.Popen([check_config_json, '-c', '-'],
                                     stdin=subprocess.PIPE,
                                     stderr=subprocess.PIPE)
            stdoutdata, stderrdata = popen.communicate(
                wrong_config.encode('utf-8'))
            assert 'No JSON object could be decoded' in stderrdata.decode(
                'utf-8')
            ...
            stdoutdata, stderrdata = popen.communicate(
                json.dumps(config).encode('utf-8'))
            assert '"dummy_setting" is not supported' in stderrdata.decode(
                'utf-8')
