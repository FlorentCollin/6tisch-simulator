    ...
    def test_context_random_seed(self, sim_engine):
        ...
        for sf_class, run_id in itertools.product(sf_class_list, [1, 2]):
            ...
            engine = sim_engine(
                ...
                run_id=run_id)
            coordinates[(sf_class,
                         run_id)] = (engine.connectivity.matrix.coordinates)
...
@pytest.fixture(params=[
    'test_setup', 'perfect_rssi', 'poor_rssi', 'worst_rssi', 'invalid_rssi'
])
...
def test_drop_ack(sim_engine, fixture_pdr):
    ...
    class TestConnectivityMatrixK7(ConnectivityMatrixK7):
        def _additional_initialization(self):
            ...
            self.set_rssi_both_directions(root.id, mote.id, channel, GOOD_RSSI)
            # dump the connectivity matrix
            print('The Connectivity Matrix ("1.0" means PDR of 100%):')
            self.dump()
    ...
    assert (len(logs) == len(
        [log for log in logs if log['packet']['type'] == 'KEEP_ALIVE']))
    ...
    if fixture_pdr == 0.0:
        ...
        raise ValueError(
            'invalid value ({0}) for fixture_pdr'.format(fixture_pdr))
