...
def test_instantiation(sim_engine, force_initial_routing_and_scheduling_state,
    ...
    sim_engine = sim_engine(
        diff_config={},
        force_initial_routing_and_scheduling_state=
        force_initial_routing_and_scheduling_state,
    )
...
def test_initial_scheduling_state(sim_engine):
    sim_engine = sim_engine(diff_config={'exec_numMotes': 10},
    ...
    for mote in reversed(sim_engine.motes):
        # check shared cell
        assert (len([
            cell for cell in mote.tsch.get_cells(None) if cell.options ==
        ...
        assert (len([
            cell for cell in mote.tsch.get_cells(parent.get_mac_addr())
            if cell.options == [d.CELLOPTION_TX]
        ]) == 1)
        # parent of "mote" one RX to "mote"
        assert (len([
            cell for cell in parent.tsch.get_cells(mote.get_mac_addr())
...
def test_run_until_everyone_joined(sim_engine, exec_num_motes,
                                   secjoin_enabled):
    sim_engine = sim_engine(
        diff_config={
    ...
    assert (len(
        [m for m in sim_engine.motes if m.secjoin.getIsJoined() is False]) > 0)
    u.run_until_everyone_joined(sim_engine)
    # everyone should have been joined
    assert (len([
        m for m in sim_engine.motes if m.secjoin.getIsJoined() is False
    ]) == 0)
