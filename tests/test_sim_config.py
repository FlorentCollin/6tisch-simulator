...
def test_generate_config(sim_engine):
    ...
    # ready to test
    config = SimConfig.generate_config(settings_dict=settings.__dict__,
...
def test_exec_minutes_per_run(sim_engine, exec_num_slotframes_per_run,
    ...
    if ((exec_num_slotframes_per_run and exec_minutes_per_run) or
        ...
        if exec_num_slotframes_per_run:
            end_asn = (diff_config['tsch_slotframeLength'] *
                       exec_num_slotframes_per_run)
        else:
            assert exec_minutes_per_run
            end_asn = (old_div(exec_minutes_per_run * 60,
                               diff_config['tsch_slotDuration']))
