...
def test_enqueue_with_priority(sim_engine):
    ...
    assert ([x['seq'] for x in mote.tsch.txQueue] == [2, 1])
    # put another "normal" packet, which will be the last packet in
    # the TX queue
    normal_packet = copy.deepcopy(base_dummy_packet)
    normal_packet['seq'] = 3
    mote.tsch.enqueue(normal_packet, priority=False)
    assert ([x['seq'] for x in mote.tsch.txQueue] == [2, 1, 3])
    ...
    mote.tsch.enqueue(priority_packet, priority=True)
    # now we have the priority packet first in the TX queue
    assert ([x['seq'] for x in mote.tsch.txQueue] == [2, 4, 1, 3])
    ...
    assert (
        mote.tsch.txQueue[-1]['seq'] == sim_engine.settings.tsch_tx_queue_size)
    ...
    assert ([x['seq'] for x in mote.tsch.txQueue[0:3]] == [2, 4, new_pkt_seq])
...
def test_retransmission_count(sim_engine):
    ...
    for channel in d.TSCH_HOPPING_SEQUENCE:
        connectivity_matrix.set_pdr_both_directions(root.id, hop1.id, channel,
                                                    0)
...
@pytest.fixture(params=[
    'no_diff', 'slot_offset', 'channel_offset', 'cell_options', 'mac_addr',
    'link_type'
])
...
def test_eb_wait_timer(sim_engine, fixture_clock_source):
    ...
    u.run_until_asn(
        sim_engine,
        (sim_engine.getAsn() +
         old_div(d.TSCH_MAX_EB_DELAY, sim_engine.settings.tsch_slotDuration) -
         1))
...
def test_desync(sim_engine, fixture_frame_from_root,
    ...
    if fixture_keep_alive_interval < 0:
        ...
        fixture_keep_alive_interval = (tsch_slotDuration *
