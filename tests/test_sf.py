...
class TestMSF(object):
    ...
    def test_autonomous_tx_cell_allocation(self, sim_engine):
        sim_engine = sim_engine(diff_config={
            'exec_numMotes': 2,
            'sf_class': 'MSF'
        })
    ...
    def test_msf(self, sim_engine):
        ...
        dio['mac'] = {'srcMac': root_mac_addr, 'dstMac': d.BROADCAST_ADDRESS}
        ...
        mote.settings.app_pkPeriod = (mote.settings.tsch_slotframeLength *
        ...
        cells = [
            cell for cell in
            mote.tsch.get_cells(mac_addr=root.get_mac_addr(),
                                slotframe_handle=SchedulingFunctionMSF.
                                SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
            if cell.options == [d.CELLOPTION_TX]
        ]
        assert len(cells) == 2
        ...
        for cell in mote.tsch.get_cells(
                root.get_mac_addr(),
    ...
    def test_no_available_cell(self, sim_engine, function_under_test):
        ...
        used_slots = hop_1.tsch.get_busy_slots(
            hop_1.sf.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
        for _slot in range(sim_engine.settings.tsch_slotframeLength):
            if _slot in used_slots:
                ...
                hop_1.tsch.addCell(slotOffset=_slot,
                                   ...
                                   slotframe_handle=hop_1.sf.
                                   SLOTFRAME_HANDLE_NEGOTIATED_CELLS)
        assert (len(
            hop_1.tsch.get_busy_slots(
                hop_1.sf.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)) ==
                sim_engine.settings.tsch_slotframeLength)
        ...
        if function_under_test == 'adapt_to_traffic':
            ...
            for relocating_cell in filter(
                    lambda cell: cell.options == [d.CELLOPTION_TX],
                    hop_1.tsch.get_cells(
                        root.get_mac_addr(),
                        hop_1.sf.SLOTFRAME_HANDLE_NEGOTIATED_CELLS)):
    ...
    def test_get_autonomous_cell(self, sim_engine):
        ...
        slot_offset, channel_offset = mote.sf._compute_autonomous_cell(
            mac_addr)
    ...
    def test_downward_traffic(self, sim_engine):
        ...
        def _schedule_app():
            sim_engine.scheduleIn(
                delay=(sim_engine.settings.tsch_slotDuration *
                       sim_engine.settings.tsch_slotframeLength),
                cb=_send_packet,
                uniqueTag='test_app',
