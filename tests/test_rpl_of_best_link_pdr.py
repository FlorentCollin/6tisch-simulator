...
def test_free_run(sim_engine):
    sim_engine = sim_engine(diff_config={'rpl_of': 'OFBestLinkPDR'})
...
def test_parent_selection(sim_engine):
    ...
    connectivity_matrix.set_pdr_both_directions(mote_0.id, mote_3.id, channel,
                                                0.0)
    ...
    connectivity_matrix.set_pdr_both_directions(
        mote_0.id, mote_2.id, channel, RplOFBestLinkPDR.ACCEPTABLE_LOWEST_PDR)
    connectivity_matrix.set_pdr_both_directions(
        mote_1.id, mote_3.id, channel, RplOFBestLinkPDR.ACCEPTABLE_LOWEST_PDR)
