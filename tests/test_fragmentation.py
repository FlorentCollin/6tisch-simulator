...
# =========================== fixtures ========================================
FRAGMENTATION = ['PerHopReassembly', 'FragmentForwarding']
...
class TestFragmentationAndReassembly(object):
    ...
    APP_PKLENGTH = list(
        range(TSCH_MAX_PAYLOAD + 1, MAX_APP_PAYLOAD_LEN, TSCH_MAX_PAYLOAD))
...
class TestMemoryManagement(object):
    ...
    def test_memory_limit(self, sim_engine, fragmentation,
                          fragmentation_ff_discard_vrb_entry_policy,
                          memory_limit):
...
class TestDatagramTagManagement(object):
    ...
    def test_datagram_tag_of_sending_packets(
        ...
        for (incoming_fragment,
             outgoing_fragment) in zip(fragments_by_leaf, hop1.tsch.txQueue):
            assert (incoming_fragment['net']['datagram_tag'] !=
...
class TestFragmentForwarding(object):
    ...
    TRIGGER_FRAGMENT_TYPE = ['missing_fragment', 'last_fragment']
    ...
    def test_discard_vrb_entry_policy(
            self, sim_engine, fragmentation_ff_discard_vrb_entry_policy,
