...
def run_until_everyone_joined(sim_engine):
    ...
    for mote in sim_engine.motes:
        ...
        mote.secjoin.setIsJoined = types.MethodType(new_setIsJoined,
                                                    mote.secjoin)
...
def run_until_mote_is_ready_for_app(sim_engine, mote):
    ...
    def new_action_receive_dio(self, packet):
        ...
        if (self.mote.tsch.getIsSync() and self.mote.secjoin.getIsJoined()):
    ...
    mote.rpl.action_receiveDIO = types.MethodType(new_action_receive_dio,
                                                  mote.rpl)
    run_until_end(sim_engine)
...
def read_log_file(filter=[], after_asn=0):
    ...
    with open(sim_settings.getOutputFile(), 'r') as f:
        ...
        for line in f:
            log = json.loads(line)
            if (log["_asn"] >= after_asn) and ((len(filter) == 0) or
                                               (log['_type'] in filter)):
                logs.append(log)
    return logs
...
def create_dio(mote):
    ...
    dio['mac'] = {'srcMac': mote.get_mac_addr(), 'dstMac': d.BROADCAST_ADDRESS}
    return dio
