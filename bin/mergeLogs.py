...
def getTargetSubDirs(logRootDir):
    ...
    for entryName in os.listdir(logRootDir):
        ...
        if (os.path.isdir(subdirPath)
                and (not startTimePattern.match(entryName))):
    ...
    for (i, j) in zip(i_list, j_list):
        ...
        if ((len(cmp.left_only) > 0) or (len(cmp.right_only) > 0)):
            raise ValueError(
                '{0} and {1} should have the same file entries'.format(
                    targetSubDirs[i], targetSubDirs[j]))
...
def getTotalTargetFileNum(targetSubDirs):
    ...
    for f in targetSubDirs:
        data_files = [
            file for file in os.listdir(f)
            if re.match('^.+\.dat$', file) != None
        ]
...
def mergeLogFiles(logDir, targetSubDirs, dryRun):
    ...
    config_json_path = os.path.join(targetSubDirs[0], 'config.json')
    ...
    for targetDir in targetSubDirs:
        ...
        for fileName in os.listdir(targetDir):
            ...
            print('[{0:3d}%] merging {1} to {2}'.format(
                int(
                    float(total_processed_file_num) / total_target_file_num *
                    100), infile_path, outfile_path))
            if not dryRun:
...
def main():
    ...
    if len(targetSubDirs) == 0:
        print('No log files to merge under {0}'.format(
    ...
    if cliparams['output_dir']:
        ...
        logDir = os.path.join(cliparams['logRootDir'],
