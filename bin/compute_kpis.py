...
def kpis_all(inputfile):
    ...
    file_settings = json.loads(
        inputfile.readline())  # first line contains settings
    # === gather raw stats
    for line in inputfile:
        ...
        if logline['_type'] == SimLog.LOG_TSCH_SYNCED['type']:
            ...
            allstats[run_id][mote_id][
                'sync_time_s'] = asn * file_settings['tsch_slotDuration']
            ...
            allstats[run_id][mote_id][
                'join_time_s'] = asn * file_settings['tsch_slotDuration']
    ...
    for (run_id, per_mote_stats) in list(allstats.items()):
        ...
        allstats[run_id]['global-stats'] = {
            'e2e-upstream-delivery': [{
                ...
                'name':
                'E2E Upstream Loss Rate',
                'unit':
                '%',
                'value': (app_packets_lost /
                          app_packets_sent if app_packets_sent > 0 else 'N/A')
            }],
            'e2e-upstream-latency': [{
                ...
                'unit':
                's',
                'mean': (mean(us_latencies) if us_latencies else 'N/A'),
                'min': (min(us_latencies) if us_latencies else 'N/A'),
                'max': (max(us_latencies) if us_latencies else 'N/A'),
                '99%':
                (np.percentile(us_latencies, 99) if us_latencies else 'N/A')
            }, {
                'name':
                'E2E Upstream Latency',
                'unit':
                ...
                'mean': (mean(us_latencies) /
                         slot_duration if us_latencies else 'N/A'),
                'min':
                (min(us_latencies) / slot_duration if us_latencies else 'N/A'),
                'max':
                (max(us_latencies) / slot_duration if us_latencies else 'N/A'),
            ...
            'current-consumed': [{
                ...
                'mA',
                'mean':
                (mean(current_consumed) if current_consumed else 'N/A'),
            ...
            'network_lifetime': [{
                ...
                'unit':
                'years',
                'min': (min(lifetimes) if lifetimes else 'N/A'),
                'total_capacity_mAh':
                BATTERY_AA_CAPACITY_mAh,
            }],
            'joining-time': [{
                'name':
                'Joining Time',
                'unit':
                'slots',
                'min': (min(joining_times) if joining_times else 'N/A'),
                'max': (max(joining_times) if joining_times else 'N/A'),
                'mean': (mean(joining_times) if joining_times else 'N/A'),
                '99%':
                (np.percentile(joining_times, 99) if joining_times else 'N/A')
            }],
            'app-packets-sent': [{
