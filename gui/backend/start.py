...
BACKEND_CONFIG_PATH_PATH = os.path.join(BACKEND_ROOTDIR_PATH,
                                        '../backend.config.json')
with open(BACKEND_CONFIG_PATH_PATH, 'r') as f:
    config = json.load(f)
SIM_ENGINE_MODULE_PATH = os.path.join(config['simulator_path'], 'SimEngine')
...
if args.auto_restart is True:
    ...
    print(
        'Start watching .py files under "{0}"'.format(SIM_ENGINE_MODULE_PATH))
