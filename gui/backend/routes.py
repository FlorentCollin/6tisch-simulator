...
@btl.get('/')
def get_index():
    return btl.static_file('index.html', root=backend.get_web_root_path())
...
def get_result_zip_file(result_zip_file_name):
    ...
    result_subdir_path = os.path.join(backend.SIM_DATA_PATH,
                                      result_subdir_name)
                ...
                for root, dirs, files in os.walk(result_subdir_path):
                    for file in files:
                        ...
                        zipf.write(filename=os.path.join(root, file),
                                   arcname=arcname)
            ret = btl.static_file(os.path.basename(tmp_file_path),
                                  root=var_dir,
