...
def test_get_default_config(default_config):
    ...
    assert config['execution'] == {"numCPUs": 1, "numRuns": 1}
...
def test_put_default_config(default_config,
    ...
    if fixture_test_put_default_config_type == 'success':
        ...
        assert ret['config']['execution'] == {"numCPUs": 1, "numRuns": 1}
...
def test_start(default_settings):
    ...
    assert (backend.sim._sim_engine.getAsn() == (
...
def test_resume(sim_engine, default_settings):
    ...
    assert _sim_engine.getAsn() == (asn_before_sleep +
                                    default_settings['tsch_slotframeLength'] -
                                    1)
...
@pytest.fixture(
    params=['success', 'failure_on_sim_state', 'failure_on_sim_existence'])
    ...
    if target_git_repo == 'webapp':
        ...
        with open(backend.BACKEND_CONFIG_PATH) as f:
            ...
            git_dir = os.path.join(
                ...
                config['simulator_path'], '.git')
    branch_name = subprocess.check_output(
        ['git', '--git-dir', git_dir, 'rev-parse', '--abbrev-ref',
         'HEAD']).strip()
    short_hash = subprocess.check_output(
        ['git', '--git-dir', git_dir, 'rev-parse', '--short=7',
         'HEAD']).strip()
